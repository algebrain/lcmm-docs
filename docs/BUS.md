...
Для полного разбора пути от HTTP-запроса к событиям, логам и аудиту см. [`TRACE_FLOW.md`](./TRACE_FLOW.md). Здесь ниже остаются только короткие опорные примеры.
...
- `:buffer-size`: для режима `:buffered`, размер очереди событий (по умолчанию `1024`).
...
Что важно для `:buffered`:

1. `:buffer-size` ограничивает число ожидающих событий, а не число подписчиков;
2. успешный `publish` означает, что событие целиком принято в буфер;
3. если буфер переполнен, `publish` сразу бросает исключение и не блокирует вызывающий поток;
4. fanout по подписчикам происходит уже после снятия события из очереди;
5. ошибка одного обработчика не мешает вызову остальных обработчиков того же события.
...
#### Поведение `publish` в режиме `:buffered`

Если шина создана с `:mode :buffered`, то:

1. в очередь ставится одно событие целиком;
2. после dequeue шина последовательно вызывает все подходящие обработчики;
3. если один обработчик падает, остальные обработчики этого события всё равно вызываются;
4. если очередь заполнена, событие не принимается вообще.
...
(bus/subscribe b :order/created handler-a)
(bus/subscribe b :order/created handler-b)
```

При успешном `publish` событие будет принято в очередь как одна запись, а затем шина вызовет и `handler-a`, и `handler-b`.
Если очередь уже заполнена, `publish` бросит исключение до начала доставки, и частичной доставки этого события не будет.
...
Ниже не новый общий порядок чтения, а только ближайшие документы по теме шины.
Если нужен полный маршрут по всей документации, ориентируйтесь на [`READING_ORDER.md`](./READING_ORDER.md).

1. [`APP_COMPOSITION.md`](./APP_COMPOSITION.md) — если нужен общий порядок сборки приложения.
2. [`LOGGING.md`](./LOGGING.md) — если нужно понять, как связывать цепочку событий в логах.
3. [`HTTP.md`](./HTTP.md) — если нужен путь от HTTP-запроса к публикации события.
4. [`TRACE_FLOW.md`](./TRACE_FLOW.md) — если нужен сквозной разбор от запроса до аудита.
5. [`TRANSACT.md`](./TRANSACT.md) — если нужен транзакционный режим публикации.
