...
1. он рассчитан на безопасный вывод результата `load-config`, то есть уже плоской карты конфигурации;
...
4. не передавайте в `dump-config` произвольные вложенные структуры приложения и не ожидайте, что он надежно скроет в них все чувствительные данные.
