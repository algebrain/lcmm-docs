...
Этот документ описывает стандартный подход к работе с базой данных в LCMM. Цель: один и тот же модуль и один и тот же набор тестов должны работать и с `SQLite`, и с основной БД (например, PostgreSQL), переключаясь одной настройкой.

## 1. Правила

*   Реализация для `SQLite` пишется первой и считается эталоном.
...
*   Один набор тестов запускается и на `SQLite`, и на основной БД, меняется только конфиг.
*   Если основной БД нет, тесты запускаются на `SQLite`. Это корректная базовая реализация.
...
Рекомендация для CI:
*   В обычном CI прогонять тесты на `SQLite`.
