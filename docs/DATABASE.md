...
Уточнения:
1. `storage.backend` выбирает backend для self-managed режима только когда внешний `:db` не передан.
2. Владение жизненным циклом ресурса должно быть однозначным:
   при `external-managed` ресурс создает и закрывает приложение;
   при `self-managed` ресурс создает и закрывает сам модуль.
