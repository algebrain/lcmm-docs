...
*   `DataScript` (пишется первой и служит эталоном для тестов и документации).
...
*   Один и тот же набор тестов должен запускаться и на `DataScript`, и на основной БД, меняется только одна настройка.
*   Если подключения к основной БД пока нет, тесты пишутся и гоняются на `DataScript`. Это считается корректной базовой реализацией.
