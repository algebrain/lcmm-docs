...
Важно держать в голове и более общую рамку:

1. registry — это узкий контрактный канал sync-read под конкретные read-case;
2. registry не должен становиться универсальным способом межмодульного доступа
   к чужим данным;
3. если через registry начинает уходить все больше кооперации между модулями, это
   уже архитектурный сигнал пересмотреть границы модулей или способ кооперации, а
   не расширять registry как основной механизм.
...
6. provider проектируется вокруг конкретного внешнего сценария, а не вокруг
   удобства внутренней модели владельца;
7. provider должен возвращать минимально достаточные данные для этого сценария;
8. provider не должен раскрывать внутреннюю модель модуля шире, чем это нужно
   потребителю;
9. если один сценарий начинает зависеть от растущего числа sync-read, это нужно
   считать архитектурным сигналом, а не нейтральной мелкой доработкой.
...
## 8. Признаки расползания связности

Ниже не новый набор запретов, а короткий практический check-list для ревью.

Тревожные сигналы:

1. у модуля быстро растет число внешних provider-зависимостей;
2. provider начинает возвращать слишком широкие структуры "на всякий случай";
3. потребитель комбинирует несколько provider-ов одного владельца, чтобы собрать
   чужую бизнес-логику;
4. новые межмодульные read-case почти автоматически решаются через registry.

Что делать в таких случаях:

1. проверить, не нужен ли provider уровня сценария вместо набора общих чтений;
2. проверить, не утекает ли наружу внутренняя модель владельца данных;
3. проверить, не должна ли логика жить у владельца данных;
4. проверить, не пора ли пересмотреть границы модулей или способ кооперации.

## 9. Связь с контрактами модулей
...
3. YAML-контракт нужен не только для совпадения `provider_id`, но и для удержания
   узкой формы входа и результата;
4. если provider начинает расползаться по смыслу или по форме возвращаемых
   данных, это должно быть видно уже на уровне контракта;
5. если поведение провайдера меняется, код и контракт обновляются вместе.
