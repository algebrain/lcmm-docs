...
Этот документ показывает короткий practical walkthrough того, как в LCMM проходит
сквозная трассировка запроса:

1. HTTP request входит в приложение;
2. появляется `correlation-id`;
3. HTTP handler публикует root event в `event-bus`;
4. reactive handler публикует производное событие;
5. audit и logs читают уже существующий trace context.
