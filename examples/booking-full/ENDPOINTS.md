...
Назначение: отдать встроенную диагностическую страницу для ручной проверки
веб-сокета в обычном браузере.
...
- `200`;
- HTML-страница;
- на странице есть кнопки для подключения, подписки и ручного вызова
  сценария создания бронирования.

### `GET /ws?user-id=<user-id>`

Назначение: websocket endpoint уровня приложения для push-доставки событий в
браузер.

Практическое уточнение:

- этот маршрут нужен не для адресной строки, а для браузерного websocket
  клиента;
- в `booking-full` он использует упрощенную demo-идентификацию по `user-id`;
- это сделано только для примера и не считается целевой моделью auth для
