...
Что делает `bb test.bb`:

1. запускает `LINT`;
2. затем запускает `TESTS`;
3. затем запускает `FORMAT`.

Первый запуск на новой машине может скачать зависимости для `clojure`-алиасов.
Форматирование входит в `bb test.bb` и может изменить файлы рабочего дерева.
