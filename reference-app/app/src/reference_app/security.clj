...
(defn make-guard [app-config]
  (let [counter-store (backend/make-counter-store)
        ...
        login-window-sec (long (config/config-value app-config "guard.login.window_sec" 300))
        login-max-failures (long (config/config-value app-config "guard.login.max_failures" 20))
...
(defn- handle-demo-login [guard-instance get-user-by-login logger request]
  (let [login (query-param request :login)]
    (cond
